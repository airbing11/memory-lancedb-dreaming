...
function selectCandidateTruths(entries, limit) {
    return [...entries]
...
export async function runRemSleep(params) {
    ...
    const entries = filterMemoriesByLookback((await params.listMemories()).filter((entry) => entry.id !== "__schema__" && entry.text.trim().length > 0), state, params.nowMs, params.config.lookbackDays)
        .sort((a, b) => b.importance - a.importance)
        .slice(0, Math.max(params.config.limit * 10, 200));
    const clusters = buildTagClusters(entries, params.config.limit, params.config.minPatternStrength);
    ...
    if (remModel && clusters.length > 0) {
        if ((params.subagent || params.llmComplete) && params.logger) {
            ...
            params.logger?.warn("memory-lancedb-dreaming: REM theme naming skipped (no subagent or llm.complete runtime available)");
        }
    }
    ...
    await recordPhaseSignals(params.workspaceDir, entries.map((entry) => entry.id), "rem", params.nowMs);
