...
function phaseBoost(entry, nowMs) {
    if (!entry)
        return 0;
    let boost = 0;
    if (entry.lightHits > 0)
        boost += Math.min(0.08, entry.lightHits * 0.025);
    if (entry.remHits > 0)
        boost += Math.min(0.08, entry.remHits * 0.025);
    const lastSeenMs = Date.parse(entry.lastSeenAt);
    if (Number.isFinite(lastSeenMs) && nowMs - lastSeenMs < 3 * DAY_MS)
        boost += 0.03;
    return boost;
}
function conceptualScore(category) {
    switch (category) {
        case "preference":
        case "decision":
            return 0.9;
...
function resolveMemoryAgeDays(memory, entry, nowMs) {
    const lastSeenMs = Date.parse(entry?.lastSeenAt ?? "");
...
function resolveContextDiversity(entry, uniqueQueries, recallDays) {
    const phaseSignals = (entry?.lightHits ?? 0) > 0 || (entry?.remHits ?? 0) > 0 ? 1 : 0;
    return Math.max(uniqueQueries, recallDays.length, phaseSignals);
}
function recencyFromAgeDays(ageDays, halfLifeDays) {
...
export function rankPromotionCandidates(params) {
    ...
    for (const memory of memories) {
        if (!memory.text?.trim() || memory.id === "__schema__")
            continue;
        ...
        const importance = clampScore(memory.importance);
        const passesRecallGate = signalCount >= config.minRecallCount ||
            (importance >= 0.72 && (lightHits > 0 || remHits > 0));
        if (!passesRecallGate)
            continue;
        if (contextDiversity < config.minUniqueQueries && importance < 0.8)
            continue;
        ...
        const score = clampScore(DEFAULT_WEIGHTS.frequency * frequency +
