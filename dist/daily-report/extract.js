...
const REM_THEME_NAMED_RE = /^-\s*Theme:\s*(.+?)\s*\/\s*(.+?)\s*\((\d+(?:\.\d+)?)\)\s*$/;
const REM_THEME_TAG_RE = /^-\s*Theme:\s*`([^`]+)` kept surfacing across (\d+) memories\.\s*$/;
const REM_CONFIDENCE_RE = /^\s*-\s*confidence:\s*(\d+(?:\.\d+)?)\s*$/;
export function parseRemThemeLines(bodyLines) {
    const themes = [];
...
export async function extractLatestNarrativeExcerpt(workspaceDir) {
    for (const filename of DREAMS_FILENAMES) {
        ...
        const diarySection = diaryStart >= 0 && diaryEnd > diaryStart
...
async function readPhaseReportLines(workspaceDir, phase, day) {
...
function countLightCandidates(lines) {
    if (lines.some((line) => line.includes("No notable updates")))
        return 0;
    return lines.filter((line) => line.startsWith("- ")).length;
}
function countDeepPromotions(lines) {
...
export async function buildSnapshotFromWorkspace(params) {
    ...
    if (existing && existing.day === params.day)
        return existing;
...
export function resolveReportDay(nowMs, timezone) {
