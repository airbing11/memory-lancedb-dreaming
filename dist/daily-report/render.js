function phaseLineZh(snapshot, phase) {
    if (phase === "light") {
        if (!snapshot.light.ran)
            return "未运行";
    ...
    if (!snapshot.deep.ran)
        return "未运行";
    return `${snapshot.deep.promotedCount} 条提升至永久记忆`;
}
function phaseLineEn(snapshot, phase) {
    if (phase === "light") {
        if (!snapshot.light.ran)
            return "not run";
...
export function renderDailyReport(snapshot, languages) {
    const langs = languages.length > 0 ? languages : ["zh"];
    const sections = [];
    if (langs.includes("zh"))
        sections.push(renderZh(snapshot));
    if (langs.includes("en"))
        sections.push(renderEn(snapshot));
