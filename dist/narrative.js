...
const dreamsFileLocks = new Map();
function buildNarrativeSessionKey(workspaceDir, nowMs, language) {
...
function resolveLanguages(config) {
    const raw = config.languages?.length ? config.languages : ["zh", "en"];
    const seen = new Set();
    const ordered = [];
...
function systemPromptForLanguage(language, mode) {
    if (mode === "snapshot") {
...
function buildNarrativePrompt(data) {
    const introZh = data.mode === "snapshot"
        ? "根据以下记忆片段写一段夜间记忆快照：\n"
        : "根据以下记忆片段写一段梦境日记：\n";
    const introEn = data.mode === "snapshot"
...
export async function generateAndAppendDreamNarrative(params) {
    if (!params.config.enabled)
        return false;
    if (params.snippets.length === 0 && params.promotions.length === 0)
        return false;
    ...
    for (const language of languages) {
        const sessionKey = buildNarrativeSessionKey(params.workspaceDir, params.nowMs, language);
    ...
    params.logger.info(`memory-lancedb-dreaming: dream diary ${params.mode} entry written (${languages.join(", ")}) [workspace=${params.workspaceDir}]`);
    return true;
}
