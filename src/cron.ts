...
export async function reconcileManagedDreamingCron(params: {
  ...
  if (dailyReportCron.collidedWithDreamingCron) {
    params.logger.warn(
      `memory-lancedb-dreaming: dailyReport.cron matches dreaming cron (${params.config.cron}); auto-staggered daily report to ${dailyReportCron.expr}`
    );
  }
...
export function resolveEffectiveDailyReportCronExpr(config: DreamingConfig): {
  expr: string;
