...
function buildLightBody(entries: LanceMemoryEntry[]): string[] {
...
export async function runLightSleep(params: {
  ...
  const recent = filterMemoriesByLookback(
    raw.filter((entry) => entry.id !== "__schema__" && entry.text.trim().length > 0),
    state,
    params.nowMs,
