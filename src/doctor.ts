...
export function evaluateDoctorChecks(input: DoctorInputs): DoctorCheck[] {
  ...
  if (input.hooksAllowConversationAccess === true) {
    ...
    checks.push({
      ...
      level: "fail",
      message:
        "hooks.allowConversationAccess is not true — the cron will idle ~120s and write no light/rem/deep files.",
  ...
  if (!input.needsModelOverride) {
    ...
    checks.push({
      ...
      level: "warn",
      message:
        "rem.model / narrative.model is set but subagent.allowModelOverride is not true — REM falls back to category labels.",
...
export async function runDreamingDoctor(input: DoctorInputs): Promise<DoctorReport> {
