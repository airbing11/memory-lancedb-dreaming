...
function needsLlmRuntime(config: DreamingConfig): boolean {
  if (config.narrative.enabled) return true;
  if (config.rem.enabled && Boolean(config.rem.model?.trim())) return true;
  return false;
}
...
export async function runDreamingPipeline(params: {
  ...
  const listMemories = (opts: { includeVector?: boolean; orderByImportance?: boolean }) =>
    withDbRead(params.db, () =>
      params.db.list(undefined, {
  ...
  let lightSnippets: string[] = [];
  let remThemes: string[] = [];
  let remBodyLines: string[] = [];
  ...
  if (params.config.light.enabled && (phase === "all" || phase === "light")) {
    ...
    params.logger.info(
      `memory-lancedb-dreaming: light sleep staged ${lightCount} candidate(s)`
    );
  }
  ...
  if (idleNoveltyAfterDays > 0) {
    try {
      ...
      if (noveltyMode) {
        params.logger.info(
          `memory-lancedb-dreaming: REM novelty mode ON (Deep idle ${idleDays}d ≥ ${idleNoveltyAfterDays}d)`
        );
      }
    } catch (err) {
      params.logger.warn(
        `memory-lancedb-dreaming: failed to read deep history for novelty mode: ${String(err)}`
      );
    }
  }
  ...
  if (params.config.rem.enabled && (phase === "all" || phase === "rem")) {
    ...
    phasesRan.rem = true;
    params.logger.info(
      `memory-lancedb-dreaming: REM sleep analyzed ${remCount} recent memories`
    );
  }

  let promotionSnippets: string[] = [];
  let rankedSnippets: string[] = [];
  ...
  if (params.config.deep.enabled && (phase === "all" || phase === "deep")) {
    ...
    params.logger.info(
      `memory-lancedb-dreaming: deep sleep promoted ${promotedCount} memory(ies) (ranked ${deep.promotions.length})`
    );
    ...
    if (params.config.narrative.enabled && (subagent || llmComplete)) {
      ...
      if (promotedCount > 0) {
        ...
        params.logger.info(
          "memory-lancedb-dreaming: snapshot narrative skipped (no novel REM themes)"
        );
      }
    }
  } else if (
    params.config.narrative.enabled &&
  ...
  if (params.config.verboseLogging) {
    params.logger.info(
      `memory-lancedb-dreaming: pipeline complete (light=${lightCount}, rem=${remCount}, promoted=${promotedCount}, narrative=${narrativeWritten})`
    );
  }
