...
export const RemDreamingConfigSchema = Type.Object({
  ...
  themeCooldownDays: Type.Number({
    ...
    maximum: 30,
    description:
      "Suppress REM theme labels that are semantically similar to themes surfaced in this many recent days",
  }),
  themeSimilarityThreshold: Type.Number({
    ...
    maximum: 1.0,
    description:
      "Similarity at or above which a REM theme is treated as a recently surfaced repeat",
  }),
  truthDedupeWindowDays: Type.Number({
    ...
    maximum: 90,
    description:
      "Look back this many days of REM history and skip lasting truths whose TEXT is semantically near an already-surfaced truth (not just same memoryId)",
  }),
  truthSimilarityThreshold: Type.Number({
    ...
    maximum: 1.0,
    description:
      "Similarity (0-1, overlap-coefficient based) at or above which a candidate truth is treated as a repeat of a recent truth and skipped. ~0.42 catches reworded same-topic truths (同一主题不同措辞) without dropping unrelated items.",
  }),
  excludePromoted: Type.Boolean({
    default: true,
    description:
      "Skip memories already promoted into MEMORY.md (state.promotedAt) when selecting lasting truths",
  }),
  model: Type.Optional(
...
export const DeepDreamingConfigSchema = Type.Object({
  ...
  idleNoveltyAfterDays: Type.Number({
    ...
    maximum: 60,
    description:
      "When this many consecutive days promote 0 memories, REM enters novelty mode: lasting truths exclude promoted/stale material and narrative stops reusing old promotion candidates. Set 0 to disable.",
  }),
});
...
export const NarrativeConfigSchema = Type.Object({
  ...
  sourceCooldownDays: Type.Number({
    ...
    maximum: 30,
    description:
      "On zero-promotion days, exclude Light snippets already used in recent narrative entries",
  }),
  sourceSimilarityThreshold: Type.Number({
    ...
    maximum: 1.0,
    description:
      "Similarity at or above which a narrative source snippet is treated as recently used",
  }),
  minNovelSnippets: Type.Number({
    ...
    maximum: 12,
    description:
      "Minimum novel Light snippets required to write a zero-promotion snapshot narrative",
  }),
  ...
  outputSimilarityThreshold: Type.Number({
    ...
    maximum: 1.0,
    description:
      "Similarity at or above which generated narrative text is skipped as a reworded repeat",
  }),
