...
export function resolveMemorySlotPluginId(
  configRoot: Record<string, unknown> | undefined
): string | undefined {
  const plugins = asRecord(configRoot?.plugins);
...
export function entryHasLancedbConfig(entry: Record<string, unknown> | undefined): boolean {
  const raw = asRecord(entry?.config);
  if (!raw) return false;
  if (asRecord(raw.embedding)) return true;
 ...
 */
export function resolveLanceDbMemoryEntryFromConfig(
  configRoot: Record<string, unknown> | undefined
